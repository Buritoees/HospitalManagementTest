...
(function initMobileRotation() {
    ...
    // Function to check if we're on a mobile device in portrait mode
    function isMobilePortrait() {
        return window.innerWidth <= 768 && 
               window.matchMedia('(orientation: portrait)').matches;
    }
    
    // Function to handle viewport resize and update rotation
    function handleViewportChange() {
        if (isMobilePortrait()) {
            // On mobile portrait, the app is rotated
            // Ensure the app wrapper dimensions are correct
            const appWrapper = document.getElementById('appWrapper');
            if (appWrapper) {
                // Force recalculation of dimensions
                appWrapper.style.width = '100vh';
                appWrapper.style.height = '100vw';
            }
            ...
            // On desktop/landscape, ensure normal behavior
            const appWrapper = document.getElementById('appWrapper');
            if (appWrapper) {
    ...
    // Handle iOS Safari viewport height changes (address bar show/hide)
    // Use visualViewport API if available
    if (window.visualViewport) {
        window.visualViewport.addEventListener('resize', handleViewportChange);
    }
    
    // Prevent unwanted zoom on double-tap for buttons and inputs
    let lastTouchEnd = 0;
    ...
    // Ensure inputs remain visible when keyboard appears
    if (isMobilePortrait()) {
        const inputs = document.querySelectorAll('input, textarea');
        inputs.forEach(input => {
            input.addEventListener('focus', function() {
                // Scroll the input into view within the rotated context
                setTimeout(() => {
